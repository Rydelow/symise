<?php class Enquiry{
	  ...
	  public static function emaildataupdate($data){
	  		 global $wpdb;
     $table_name = $wpdb->prefix.'enquiry_emailsetting';
     $wpdb->query("UPDATE $table_name SET email='".$_POST['email']."' WHERE id='1'");
    ...
    add_submenu_page('all_enquiry', 'Receive Mail-id', 'Receive Mail-id', 'manage_options', 'enquiry_email_data',[$this,'enquiry_email_setting']);
		}
...
<div class="row">
    <div class="col-md-10">
    		 ...
    		 <div class="row page_height">
                <div class="col-md-2">
                    <label class="control-label" for="url">Add Receive Mail-id</label>
                </div>	  
                <div class="col-md-6">
    ...
    $(document).ready(function(){
        $("#emailsetting").click(function(){
           ...
           var email=$('#emailiddata').val();
            ...
            $.ajax({
      type: 'POST',
      url: "<?php echo site_url('/wp-content/plugins/symise/email_setting.php'); ?>",
    data: "email="+email+"&wsfunction=emailsettingdata",
      dataType: "text",
...
<div class="row">
    ...
    $table_name = $wpdb->prefix.'equirydata';
    $emaildata=$wpdb->get_results("select * from $table_name order by id desc");
    $i=1;
    foreach ($emaildata as $userValue) {
      date_default_timezone_set('asia/kolkata');
     
      $en_time = date('d-M-Y h:i:s a',$userValue->createddate);  
  ...
  <p style='font-size:14px;'><b>Name : </b><span><?php echo $userValue->name; ?></span></p>

  <p style='font-size:14px;'><b>Email-id : </b><span><?php echo $userValue->email; ?></span></p>

<p style='font-size:14px;'><b>Subject : </b><span><?php echo $userValue->subject; ?></span></p>
 <p style='font-size:14px;'><b>Messge : </b><span><?php echo $userValue->message; ?></span></p>
 <p style='font-size:14px;'><b>Ip : </b><span><?php echo $userValue->ip; ?></span></p>
 <p style='font-size:14px;'><b>Brower Details : </b><span><?php echo $userValue->brower_details; ?></span></p>
...
<div class="row">
    ...
    $table_name = $wpdb->prefix.'equirydata';
    $emaildata=$wpdb->get_results("select * from $table_name where `date`='".date('Y-m-d')."' order by id desc");
    $i=1;
    foreach ($emaildata as $userValue) {
      date_default_timezone_set('asia/kolkata');
